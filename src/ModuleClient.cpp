...
void ModuleClient::onPacketReceivedQueryAllMessagesResponse(const InputMemoryStream & stream)
{
	...
	// TODO7: Deserialize the number of messages
	...
	for (int i = 0; i < messageCount; i++)
	{
		...
		stream.Read(aux.receiverUsername);
		stream.Read(aux.subject);
		stream.Read(aux.body);
		stream.Read(aux.senderUsername);
...
void ModuleClient::sendPacketQueryMessages()
{
	...
	// TODO4: Serialize message (only the packet type)
...
void ModuleClient::sendPacketSendMessage(const char * receiver, const char * subject, const char *message)
{
	...
	// TODO8: Serialize message (packet type and all fields in the message)
