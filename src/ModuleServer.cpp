...
int ModuleServer::onPacketReceived(SOCKET socket, const InputMemoryStream & stream)
{
	...
	// TODO2: Deserialize the packet type
...
void ModuleServer::sendPacketQueryAllMessagesResponse(SOCKET socket, const std::string &username)
{
	...
	// TODO6: Create QueryAllMessagesResponse and serialize all the messages

	// -- serialize the packet type
	outStream.Write(PacketType::QueryAllMessagesResponse);

	// -- serialize the array size
	outStream.Write((uint32_t)messages.size());
...
{
	Message message;
	// TODO9: Deserialize the packet (all fields in Message)
